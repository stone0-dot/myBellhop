...
! **********************************************************************!
   SUBROUTINE CREADBTY( BotBTY, DepthB, rBox, PRTFil, c_NbtyPts, c_btyPts)

      ! Reads in the bottom bathymetry
      USE iso_c_binding
      INTEGER     PRTFil
      CHARACTER*1 BotBTY
      integer(4), intent(in) :: c_NbtyPts
      type(c_ptr), intent(in) :: c_btyPts
      real(8), pointer :: f_btyPts(:, :)
      ...
      IF ( BotBTY == '*' ) THEN
         ...
         call c_f_pointer(c_btyPts, f_btyPts, [2, c_NbtyPts])
         DO I = 1, NbtyPts
            xBot( :, I ) = f_btyPts(: , I)
