SUBROUTINE CREADIN( Title, freq, ISINGL, &
   Nimage, Ibwin, deltas, MxN, zBox, rBox, epmult, rLoop, &
   ...
   CHARACTER    Title*80, PltTyp*10, TopOpt*5, BotOpt*3, RunType*4, BeamType*3, SSPType*1, BCType*1, AtUnit*1, FileName*6

   real(8), intent(in) :: f_alphaR, f_betaR, f_rhoR, f_alphaI, f_betaI
   integer(c_int), intent(in) :: c_NSD, c_NRD, c_NR
   ...
   integer(4), intent(in) :: c_zSSPV_len, c_cSSPV_len, c_SD_len, c_RD_len, c_R_len, c_alpha_len,&
      c_NBeams
   real(4), pointer :: f_cSSPV(:), f_zSSPV(:), f_alpha(:)
   ...
   CALL CTOPBOT( ENVFil, PRTFil, freqD, BCType, AtUnit, cpT, csT, rhoT, BumDen, eta, xi )
   ...
   ELSE
      ...
      call c_f_pointer(c_zSSPV, f_zSSPV, [c_zSSPV_len])
      call c_f_pointer(c_cSSPV, f_cSSPV, [c_cSSPV_len])
      ZSSPV(1:c_zSSPV_len) = f_zSSPV(1:c_zSSPV_len)
      CSSPV(1:c_cSSPV_len) = f_cSSPV(1:c_cSSPV_len)
      CALL CSSP( x, C, gradC, CRR, CRZ, CZZ, SSPType, 'INI' )
   ENDIF

   DepthT = zSSPV(1)   ! Depth of top boundary is taken from first SSP point
   ...
   betaI = f_betaI
   rhoR = f_rhoR
   CALL CTOPBOT( ENVFil, PRTFil, freqD, BotOpt, AtUnit, cpB, csB, rhoB, BumDen, eta, xi )
   CALL CSDRD(   ENVFil, PRTFil, DepthT, DepthB, c_SD, c_SD_len, c_RD, c_RD_len, c_NSD, c_NRD)   ! *** Read source/receiver Depths ***
   CALL CRANGES( ENVFil, PRTFil, c_R, c_R_len, c_NR)                   ! *** Read receiver ranges ***
   ...
   call c_f_pointer(c_alpha, f_alpha, [c_alpha_len])
   alpha(1:c_alpha_len) = f_alpha(1:c_alpha_len)
   CALL SUBTAB( alpha, NBeams )
   CALL SORT(   alpha, NBeams )

   WRITE( PRTFil, * )
   WRITE( PRTFil, * ) 'Number of beams   = ', NBeams
   IF ( ISINGL > 0 ) WRITE( PRTFil, * ) 'Trace only beam number ', ISINGL
   ...
   ELSE
      ...
      WRITE( PRTFil, * ) 'Beam windowing parameter,  Ibwin = ', Ibwin
   ...
   ELSE IF ( RunType(1:1) == 'A' ) THEN
      ...
      WRITE( ARRFil, * ) ( SD( I ),  I = 1, NSD )
      WRITE( ARRFil, * ) ( RD( I ),  I = 1, NRD )
      WRITE( ARRFil, * ) ( R( IR ), IR = 1, NR  )
   ...
   ELSE
      FileName = 'SHDFIL'
      PltTyp = ''
      CALL WriteHeader( FileName, Title, SD, NSD, RD, NRD, R, NR, freq, 0.0, PltTyp, 0.0, 0.0, 0.0 )
   END IF

   RETURN
END SUBROUTINE CREADIN
!*************************************************************************************
SUBROUTINE READIN( Title, freq, ISINGL, &
   Nimage, Ibwin, deltas, MxN, zBox, rBox, epmult, rLoop, &
   TopOpt, DepthT, cpT, rhoT, BotOpt, DepthB, cpB, rhoB, RunType, BeamType )
   ...
   CALL SDRD(   ENVFil, PRTFil, DepthT, DepthB )   ! *** Read source/receiver Depths ***
   CALL RANGES( ENVFil, PRTFil )                   ! *** Read receiver ranges ***

   ! *** Read run type ***

   READ(  ENVFil, * ) RunType(1:4)
   WRITE( PRTFil, * )
   ...
   IF ( TopOpt( 5:5 ) == 'I' ) THEN
      READ( ENVFil, * ) NBeams, ISINGL     ! hidden option to trace a single beam
   ELSE
      READ( ENVFil, * ) NBeams
   END IF
...
!**********************************************************************!
SUBROUTINE CSSP( x, C, gradC, CRR, CRZ, CZZ, OPT, Task )
   ...
   SELECT CASE ( OPT )
    CASE ( 'N' )
      CALL CN2LIN(  x, C, gradC, CRR, CRZ, CZZ, Task )
    CASE ( 'C' )
      CALL CCLIN(   x, C, gradC, CRR, CRZ, CZZ, Task )
    CASE ( 'S' )
      CALL CCCUBIC( x, C, gradC, CRR, CRZ, CZZ, Task )
    CASE ( 'A' )
...
!**********************************************************************!
SUBROUTINE CN2LIN( x, C, gradC, CRR, CRZ, CZZ, Task )
   ...
   IF ( Task == 'INI' ) THEN
      ...
      WRITE( PRTFil, * ) 'Max. #SSP points: ', MxSSP
...
!**********************************************************************!
SUBROUTINE CCLIN( x, C, gradC, CRR, CRZ, CZZ, Task )
...
!**********************************************************************!
SUBROUTINE CCCUBIC( x, C, gradC, CRR, CRZ, CZZ, Task )
...
!**********************************************************************!
SUBROUTINE CTOPBOT( ENVFil, PRTFil, freq, BCType, AtUnit, cpHS, csHS, rhoHS, f_BumDen, f_eta, f_xi)
   ...
   real(8), intent(in) :: f_BumDen, f_eta, f_xi
