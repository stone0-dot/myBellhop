  ...
  DO I = 2, Nsteps  ! Loop over steps
     ...
     DO WHILE ( ABS( RB - RA ) > TINY( RA ) .AND. RB > r( ir ) )  ! Loop over bracketted receiver ranges
        ...
        DO id = 1, Nrd  ! Loop over receiver depths
           ...
           n      = abs( n )
           q      = qV( 1, I-1 ) + s * ( qV( 1, I ) -  qV( 1, I-1 ) )       ! amplitude
           RadMax = ABS( q / q0 )          	! beam radius

           IF ( n < RadMax ) THEN
              A        = 1 / RadMax
              c        =    cV( I-1 ) + s * (   cV( I ) -   cV( I-1 ) )       ! sound speed
              delay    =  tauV( I-1 ) + s * ( tauV( I ) - tauV( I-1 ) )       ! delay
              ...
              CASE ( 'A' )      ! arrivals
                 ...
                 CALL AddArr( omega, id, ir, Amp, PhaseV( I - 1 ) + phaseInt, delay, &
                      SrcAngle, RcvrAngle, NumTopBnc( I ), NumBotBnc( I ) )
              CASE ( 'C'  )     ! coherent TL
                 U( id, ir ) = U( id, ir ) + Amp * EXP( -CI * ( omega * delay - PhaseV( I - 1 ) - phaseInt ) )
  ...
  DO I = 2, Nsteps   ! *** Loop over steps ***
     ...
     DO WHILE ( ABS( RB - RA ) > TINY( RA ) .AND. RB > r( ir ) )   ! Loop over bracketted receiver ranges
        ...
        DO id = 1, Nrd  ! Loop over receiver depths
           ...
           lambda = cV( I - 1 ) / ( omega / ( 2 * pi ) )
           sigma  = MAX( sigma, MIN( r(ir) * dalpha , pi * lambda ) )

           IF ( n < IBWin * sigma ) THEN   ! Within beam window?
              A        = abs( q0 / q )
              c        =    cV( I-1 ) + s * (   cV( I ) -   cV( I-1 ) )       ! sound speed
              delay    =  tauV( I-1 ) + s * ( tauV( I ) - tauV( I-1 ) )       ! delay
              ...
              CASE ( 'A' )                ! arrivals
                 RcvrAngle  = RadDeg * ATAN2( tray(2), tray(1) )
                 CALL AddArr( omega, id, ir, Amp, PhaseV( I ) + phaseInt, delay, &
                      SrcAngle, RcvrAngle, NumTopBnc( I ), NumBotBnc( I ) )
              CASE( 'C' )                 ! coherent TL
                 U( id, ir ) = U( id, ir ) + Amp * EXP( -CI * ( omega * delay - PhaseV( I ) - phaseInt ) )
